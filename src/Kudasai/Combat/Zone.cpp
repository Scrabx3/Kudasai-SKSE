...
namespace Kudasai
{
	bool Zone::registerdefeat(RE::Actor* victim, RE::Actor* aggressor)
	{
		...
		if (!agrzone || !viczone) {
			logger::warn("Failed to register defeat, associated Combat Group missing");
			return false;
		}
		...
		switch (getdefeattype(agrzone)) {
		case DefeatResult::Cancel:
			return false;
		default:
			{
				_m.lock();
	...
	void Zone::assault(RE::Actor* victim, RE::Actor* aggressor)
	{
		...
		std::this_thread::sleep_for(std::chrono::microseconds(300));
		if (victim->IsDead() && !Defeat::isdefeated(victim)) {
			_m.unlock();
			return;
		}
